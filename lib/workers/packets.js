...
class SignPacket extends Packet {
  ...
  write(bw) {
    ...
    for (const ring of this.rings) ring.write(bw);
...
class SignResultPacket extends Packet {
  ...
  fromTX(tx, total) {
    ...
    for (const input of tx.inputs) this.witness.push(input.witness);
  ...
  write(bw) {
    ...
    for (let i = 0; i < this.witness.length; i++) this.witness[i].write(bw);
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++) this.witness.push(Witness.read(br));
