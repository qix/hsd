...
const { DNSServer, hsig, wire, util } = bns;
...
class RootServer extends DNSServer {
  ...
  async lookupName(name) {
    if (!this.lookup) throw new Error("Tree not available.");

    if (!rules.verifyName(name)) return null;
    ...
    if (ns.data.length === 0) return null;
  ...
  async response(req, rinfo) {
    ...
    const data = !blacklist.has(tld) ? await this.lookupName(tld) : null;
  ...
  async resolve(req, rinfo) {
    ...
    if (!util.equal(tld, "_synth")) this.cache.set(name, type, res);
...
function toKey(name, type) {
  ...
  if (labels.length > 1) return label.toLowerCase();
