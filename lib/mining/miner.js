...
class Miner extends EventEmitter {
  ...
  updateTime(attempt) {
    ...
    if (!pow.targetReset) return;

    const prev = this.chain.tip;

    if (!attempt.prevBlock.equals(prev.hash)) return;
  ...
  assemble(attempt) {
    ...
    while (pq.size() > 0) {
      if (attempt.claims.length >= 10) break;

      const item = pq.shift();
      const weight = item.getWeight();

      if (attempt.weight + weight > this.options.maxWeight) continue;

      if (attempt.updates + 1 > this.options.maxUpdates) continue;
    ...
    while (pqa.size() > 0) {
      if (attempt.airdrops.length >= 10) break;
    ...
    while (queue.size() > 0) {
      ...
      if (!tx.isFinal(attempt.height, attempt.mtp)) continue;
      ...
      if (opens > this.options.maxOpens) continue;

      updates += rules.countUpdates(tx);

      if (updates > this.options.maxUpdates) continue;

      renewals += rules.countRenewals(tx);

      if (renewals > this.options.maxRenewals) continue;
...
class MinerOptions {
  ...
  fromOptions(options) {
    ...
    if (options.maxOpens != null) {
      ...
      assert(
        options.maxOpens <= consensus.MAX_BLOCK_OPENS,
    ...
    if (options.maxUpdates != null) {
      ...
      assert(
        options.maxUpdates <= consensus.MAX_BLOCK_UPDATES,
    ...
    if (options.maxRenewals != null) {
      ...
      assert(
        options.maxRenewals <= consensus.MAX_BLOCK_RENEWALS,
