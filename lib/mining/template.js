...
class BlockTemplate {
  ...
  createCoinbase() {
    ...
    assert(input.witness.getSize() <= 1000, "Coinbase witness is too large!");
  ...
  refresh() {
...

    {
      ...
      for (const { tx } of this.items) leaves.push(tx.hash());
  ...
  addTX(tx, view) {
    ...
    if (!tx.isFinal(this.height, this.mtp)) return false;

    if (this.weight + weight > consensus.MAX_BLOCK_WEIGHT) return false;

    if (this.sigops + sigops > consensus.MAX_BLOCK_SIGOPS) return false;

    if (this.opens + opens > consensus.MAX_BLOCK_OPENS) return false;

    if (this.updates + updates > consensus.MAX_BLOCK_UPDATES) return false;

    if (this.renewals + renewals > consensus.MAX_BLOCK_RENEWALS) return false;
...
class BlockProof {
  ...
  powHash() {
    ...
    for (let i = 0; i < 32; i++) hash[i] ^= this.mask[i];
