...
async function _resolve(name, family) {
  ...
  if (family == null || family === 4) types.push(wire.types.A);

  if (family == null || family === 6) types.push(wire.types.AAAA);
  ...
  for (const type of types) {
    ...
    for (const rr of res.collect(name, type)) out.push(rr.data.address);
  }
