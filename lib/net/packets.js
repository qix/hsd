...
class AddrPacket extends Packet {
  ...
  getSize() {
    ...
    for (const addr of this.items) size += addr.getSize();
    return size;
  }
  ...
  write(bw) {
    ...
    for (const item of this.items) item.write(bw);
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++) this.items.push(NetAddress.read(br));
...
class InvPacket extends Packet {
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++) this.items.push(InvItem.read(br));
...
class HeadersPacket extends Packet {
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++) this.items.push(Headers.read(br));
...
class RejectPacket extends Packet {
  ...
  fromOptions(options) {
    ...
    if (options.message != null) this.message = options.message | 0;
