...
class Peer extends EventEmitter {
  ...
  announceTX(txs) {
    ...
    for (const tx of txs) {
      ...
      if (this.spvFilter) {
        if (!tx.test(this.spvFilter)) continue;
      }
  ...
  announceClaim(claims) {
    ...
    for (const claim of claims) {
      ...
      // Don't send if they already have it.
      if (this.invFilter.test(claim.hash())) continue;
  ...
  announceAirdrop(proofs) {
    ...
    for (const proof of proofs) {
      ...
      // Don't send if they already have it.
      if (this.invFilter.test(proof.hash())) continue;
  ...
  write(data) {
    ...
    if (this.stream.write(data) === false) this.needsDrain(data.length);
  }
  ...
  blockType() {
    ...
    if (this.options.compact && this.hasCompact()) return invTypes.CMPCT_BLOCK;
  ...
  sendGetHeaders(locator, stop) {
    ...
    this.logger.debug("Sending getheaders (hash=%x, stop=%x).", hash, stop);
  ...
  sendGetBlocks(locator, stop) {
    ...
    if (hash.equals(this.lastTip) && stop.equals(this.lastStop)) return;
    ...
    this.logger.debug("Sending getblocks (hash=%x, stop=%x).", hash, stop);
  ...
  sendReject(code, reason, msg, hash) {
    ...
    if (msg != null) {
      this.logger.debug(
        "Rejecting %s %x (%s): code=%s reason=%s.",
  ...
  sendProof(root, key, proof) {
    this.logger.info("Sending proof (%s).", this.hostname());
