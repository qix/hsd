...
class CipherState extends EventEmitter {
  ...
  encrypt(pt, ad) {
    ...
    if (this.nonce === ROTATION_INTERVAL) this.rotateKey();

    return tag;
  }

  decrypt(ct, tag, ad) {
    if (!aead.decrypt(this.key, this.iv, ct, tag, ad)) return false;
...
class SymmetricState extends CipherState {
  ...
  decryptHash(ct, tag) {
    ...
    if (!this.decrypt(ct, tag, this.digest)) return false;
...
class Brontide extends HandshakeState {
  ...
  recvActOne(actOne) {
    ...
    if (actOne.length !== ACT_ONE_SIZE) throw new Error("Act one: bad size.");
    ...
    if (!this.decryptHash(EMPTY, p)) throw new Error("Act one: bad tag.");
  ...
  recvActTwo(actTwo) {
    ...
    if (actTwo.length !== ACT_TWO_SIZE) throw new Error("Act two: bad size.");
    ...
    if (!this.decryptHash(EMPTY, p)) throw new Error("Act two: bad tag.");
  ...
  recvActThree(actThree) {
    ...
    // s
    if (!this.decryptHash(s1, p1)) throw new Error("Act three: bad tag.");
    ...
    if (!this.decryptHash(s2, p2)) throw new Error("Act three: bad tag.");
...
class BrontideStream extends Brontide {
  ...
  unleash() {
    ...
    for (const buf of this.buffer) this.write(buf);
  ...
  write(data) {
    ...
    if (this.state === ACT_NONE) return false;
  ...
  _parse(data) {
    ...
    if (!this.hasSize) {
      ...
      if (size > MAX_MESSAGE) throw new Error("Bad packet size.");
