...
class Pool extends EventEmitter {
  ...
  resolveHeight(hash, height) {
    ...
    for (let peer = this.peers.head(); peer; peer = peer.next) {
      if (!peer.bestHash.equals(hash)) continue;
  ...
  announceList(peer) {
    ...
    if (claims.length > 0) peer.announceClaim(claims);

    if (proofs.length > 0) peer.announceAirdrop(proofs);
  }
  ...
  async sendBlock(peer, item) {
    ...
    if (this.chain.options.spv || this.chain.options.prune) {
      return false;
    }
  ...
  async handleAddr(peer, packet) {
    ...
    for (const addr of addrs) {
      ...
      if (this.hosts.isBanned(addr.host)) continue;

      if (addr.time <= 100000000 || addr.time > now + 10 * 60)
        addr.time = now - 5 * 24 * 60 * 60;

      if (!peer.gettingAddr && addrs.length < 10) {
        if (addr.time > since) relay.push(addr);
      }

      this.hosts.add(addr, peer.address);
    }

    if (addrs.length < 1000) peer.gettingAddr = false;
    ...
    if (relay.length > 0) {
      ...
      for (let peer = this.peers.head(); peer; peer = peer.next) {
        if (peer.handshake) peers.push(peer);
      }
      ...
      if (peers.length > 0) {
        for (const addr of relay) {
          ...
          if (peer1.addrFilter.added(key)) peer1.send(msg);

          if (peer2.addrFilter.added(key)) peer2.send(msg);
        }
      }
    }
  ...
  async _handleInv(peer, packet) {
    ...
    if (claims.length > 0) await this.handleClaimInv(peer, claims);

    if (proofs.length > 0) await this.handleAirdropInv(peer, proofs);
  }
  ...
  async handleGetBlocks(peer, packet) {
    ...
    while (hash) {
      if (hash.equals(packet.stop)) break;
  ...
  async handleGetHeaders(peer, packet) {
    ...
    while (entry) {
      ...
      if (entry.hash.equals(packet.stop)) break;
  ...
  getHost() {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (this.hosts.local.has(addr.hostname)) continue;
      ...
      if (this.options.brontideOnly && !addr.hasKey()) continue;
  ...
  removePeer(peer) {
    ...
    for (const hash of peer.claimMap.keys()) this.resolveClaim(peer, hash);

    for (const hash of peer.airdropMap.keys()) this.resolveAirdrop(peer, hash);
  ...
  getClaim(peer, hashes) {
    ...
    for (const hash of hashes) {
      if (this.claimMap.has(hash)) continue;
  ...
  getAirdrop(peer, hashes) {
    ...
    for (const hash of hashes) {
      if (this.airdropMap.has(hash)) continue;
  ...
  ensureClaim(peer, hashes) {
    ...
    for (const hash of hashes) {
      if (this.hasClaim(hash)) continue;
  ...
  ensureAirdrop(peer, hashes) {
    ...
    for (const hash of hashes) {
      if (this.hasAirdrop(hash)) continue;
  ...
  resolveClaim(peer, hash) {
    if (!peer.claimMap.has(hash)) return false;
  ...
  resolveAirdrop(peer, hash) {
    if (!peer.airdropMap.has(hash)) return false;
  ...
  resolveItem(peer, item) {
    ...
    if (item.isClaim()) return this.resolveClaim(peer, item.hash);

    if (item.isAirdrop()) return this.resolveAirdrop(peer, item.hash);
  ...
  pickProver(nameHash) {
    ...
    for (peer = this.peers.head(); peer; peer = peer.next) {
      if (!peer.ack || !(peer.services & common.services.NETWORK)) continue;

      if (
        peer.totalProofs > firstBest.totalProofs &&
    ...
    if (rand && random(5) === 0) return rand;

    if (secondBest && random(10) === 0) return secondBest;

    if (firstBest && random(10) === 0) return firstBest;
  ...
  async resolve(nameHash) {
    ...
    if (!this.chain.synced) throw new Error("Chain is not synced.");
    ...
    if (!peer) throw new Error("No peers available.");
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.host != null) this.host = IP.normalize(options.host);
    ...
    if (options.publicHost != null) {
      ...
      if (!IP.isRoutable(raw)) throw new Error("Invalid public host.");
    ...
    if (options.publicBrontidePort != null) {
      assert(
        (options.publicBrontidePort & 0xffff) === options.publicBrontidePort
      );
