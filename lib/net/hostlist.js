...
class HostList {
  ...
  init() {
    for (let i = 0; i < this.maxFreshBuckets; i++) this.fresh.push(new Map());

    for (let i = 0; i < this.maxUsedBuckets; i++) this.used.push(new List());
  }
  ...
  injectSeeds() {
    ...
    for (const node of nodes) {
      ...
      if (this.map.has(addr.hostname)) continue;
  ...
  add(addr, src) {
    ...
    if (bucket.size >= this.maxEntries) this.evictFresh(bucket);
  ...
  markAck(hostname, services) {
    ...
    if (fresh.size >= this.maxEntries) fresh = old;
  ...
  toArray() {
    ...
    for (const entry of this.map.values()) items.push(entry);
    ...
    for (let i = 0; i < items.length && out.length < 2500; i++) {
      ...
      if (!this.isStale(entry)) out.push(entry.addr);
    }
  ...
  getLocal(src) {
    ...
    for (const dest of this.local.values()) {
      if (this.network.type === "main") {
        ...
        if (dest.type < HostList.scores.UPNP) continue;
      }
    ...
    if (bestDest) bestDest.time = this.network.now();
  ...
  async populate(target) {
    ...
    for (const host of hosts) {
      const addr = NetAddress.fromHost(
        host,
        this.network.port,
        null,
        this.network
  ...
  fromJSON(json) {
    ...
    assert(json.fresh.length <= this.maxFreshBuckets, "Buckets mismatch.");
    ...
    for (const keys of json.fresh) {
      ...
      assert(bucket.size <= this.maxEntries, "Bucket size mismatch.");
    ...
    assert(json.used.length <= this.maxUsedBuckets, "Buckets mismatch.");
...
function groupKey(raw) {
  ...
  if (bits > 0) out[i++] = raw[start] | ((1 << (8 - bits)) - 1);
