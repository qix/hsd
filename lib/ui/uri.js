...
class URI {
  ...
  fromString(str, network) {
    ...
    assert(str.length <= 1000, "Invalid URI.");
    ...
    assert(prefix === pkg.currency, "Invalid URI.");
  ...
  toString() {
    let str = `${pkg.currency}:`;

    str += this.address.toString();

    const query = [];

    if (this.amount !== -1) query.push(`amount=${Amount.coin(this.amount)}`);
