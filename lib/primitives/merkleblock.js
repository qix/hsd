...
class MerkleBlock extends AbstractBlock {
  ...
  fromJSON(json) {
    ...
    for (const hash of json.hashes) this.hashes.push(Buffer.from(hash, "hex"));
  ...
  static fromBlock(block, filter) {
    ...
    for (const tx of block.txs) matches.push(tx.test(filter) ? 1 : 0);
  ...
  static fromMatches(block, matches) {
    ...
    const hash = (height, pos, leaves) => {
      if (height === 0) return merkle.hashLeaf(blake2b, leaves[pos]);
