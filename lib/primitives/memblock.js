...
class MemBlock extends AbstractBlock {
  ...
  powHash() {
    ...
    const mask = this._raw.slice(
      consensus.HEADER_SIZE - 32,
      consensus.HEADER_SIZE
    );

    for (let i = 0; i < 32; i++) hash[i] ^= mask[i];
  ...
  parseCoinbaseHeight() {
    ...
    for (let i = 0; i < inCount; i++) Input.read(br);

    const outCount = br.readVarint();

    for (let i = 0; i < outCount; i++) Output.read(br);
