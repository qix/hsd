...
class Output extends bio.Struct {
  ...
  fromOptions(options) {
    ...
    if (options.covenant) this.covenant.fromOptions(options.covenant);
  ...
  equals(output) {
    assert(output instanceof this.constructor);
    return this.value === output.value && this.address.equals(output.address);
  }
  ...
  getDustThreshold(rate) {
    if (!this.covenant.isDustworthy()) return 0;
  ...
  fromJSON(json) {
    ...
    if (json.covenant != null) this.covenant.fromJSON(json.covenant);
