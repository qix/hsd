...
class Covenant extends bio.Struct {
  ...
  constructor(type, items) {
    ...
    if (type != null) this.fromOptions(type, items);
  }
  ...
  fromOptions(type, items) {
    ...
    if (Array.isArray(type)) return this.fromArray(type);

    if (type != null) {
      assert((type & 0xff) === type);
      this.type = type;
      if (items) return this.fromArray(items);
      return this;
    }
  ...
  isName() {
    if (this.type < types.CLAIM) return false;

    if (this.type > types.REVOKE) return false;
  ...
  getJSON() {
    ...
    for (const item of this.items) items.push(item.toString("hex"));
  ...
  fromString(items) {
    ...
    for (const item of items) this.items.push(util.parseHex(item, -1));
