...
class TX extends bio.Struct {
    ...
    if (this.isCoinbase()) {
      for (let i = 1; i < this.inputs.length; i++) {
        ...
        if (!output.covenant.isClaim()) {
          ...
          if (
            output.value !== proof.getValue() - proof.fee ||
  ...
  checkInput(index, coin, flags) {
    ...
    Script.verify(input.witness, coin.address, this, index, coin.value, flags);
  }
  ...
  isFinal(height, time) {
    ...
    if (this.locktime & FLAG) {
      ...
      if (locktime * MULT < time) return true;
    } else {
      if (this.locktime < height) return true;
    }
  ...
  verifyLocktime(index, predicate) {
    ...
    if ((this.locktime & FLAG) !== (predicate & FLAG)) return false;

    if ((predicate & MASK) > (this.locktime & MASK)) return false;
  ...
  checkSanity() {
    ...
    if (this.isCoinbase()) {
      ...
      if (size > 1000) return [false, "bad-cb-length", 100];

      for (let i = 1; i < this.inputs.length; i++) {
        const input = this.inputs[i];

        if (!input.prevout.isNull()) return [false, "bad-cb-outpoint", 100];
        ...
        if (size > 10000) return [false, "bad-cb-length", 100];
      }
    } else {
    ...
    if (!this.hasSaneCovenants()) return [false, "bad-txns-covenants", 100];
  ...
  checkStandard() {
    if (this.version > policy.MAX_TX_VERSION) return [false, "version", 0];
    ...
    for (const output of this.outputs) {
      if (output.address.isUnknown()) return [false, "address", 0];
      ...
      if (output.covenant.isUnknown()) return [false, "covenant", 0];
  ...
  hasStandardInputs(view) {
    ...
    for (const input of this.inputs) {
      ...
      if (addr.isPubkeyhash()) {
        ...
        if (witness.items[0].length !== 65) return false;

        if (witness.items[1].length !== 33) return false;
      ...
      if (addr.isScripthash()) {
        ...
        if (m !== -1) {
          ...
          for (let i = 1; i < witness.items.length - 1; i++) {
            const item = witness.items[i];
            if (item.length !== 65) return false;
          }
        }
  ...
  checkInputs(view, height, network) {
    ...
    if (this.isCoinbase()) {
      ...
      if (conjured === -1) return [-1, "bad-txns-claims", 100];
    ...
    if (conjured === -1) return [-1, "bad-txns-covenants", 100];
