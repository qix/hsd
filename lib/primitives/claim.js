...
class Claim extends bio.Struct {
  ...
  hash() {
    if (!this._hash) this._hash = blake2b.digest(this.blob);
  ...
  decode(data) {
    ...
    if (data.length > 2 + 10000) throw new Error("Proof too large.");
  ...
  read(br) {
    ...
    if (size > 10000) throw new Error("Invalid claim size.");
