...
class AbstractBlock extends bio.Struct {
  ...
  parseOptions(options) {
    ...
    assert(
      Buffer.isBuffer(options.extraNonce) &&
  ...
  hash() {
    if (this.mutable) return this.powHash();

    if (!this._hash) this._hash = this.powHash();
  ...
  maskHash() {
    if (this._maskHash != null) return this._maskHash;
