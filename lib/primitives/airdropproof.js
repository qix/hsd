...
class AirdropProof extends bio.Struct {
  ...
  getSize(sighash = false) {
    ...
    if (!sighash) size += bio.sizeVarBytes(this.signature);

    return size;
  }

  write(bw, sighash = false) {
    if (sighash) bw.writeBytes(CONTEXT);

    bw.writeU32(this.index);
    bw.writeU8(this.proof.length);

    for (const hash of this.proof) bw.writeBytes(hash);

    bw.writeU8(this.subindex);
    bw.writeU8(this.subproof.length);

    for (const hash of this.subproof) bw.writeBytes(hash);
    ...
    if (!sighash) bw.writeVarBytes(this.signature);
  ...
  decode(data) {
    ...
    if (data.length > MAX_PROOF_SIZE) throw new Error("Proof too large.");
  ...
  verifyMerkle(expect) {
    if (expect == null) {
      expect = this.isAddress() ? FAUCET_ROOT : AIRDROP_ROOT;
    }
  ...
  verifySignature() {
    ...
    if (key.isAddress()) {
      const fee = key.sponsor ? SPONSOR_FEE : RECIPIENT_FEE;
  ...
  getValue() {
    if (!this.isAddress()) return AIRDROP_REWARD;
  ...
  isSane() {
    ...
    if (this.version > 31) return false;

    if (this.address.length < 2 || this.address.length > 40) return false;

    const value = this.getValue();

    if (value < 0 || value > consensus.MAX_MONEY) return false;

    if (this.fee < 0 || this.fee > value) return false;

    if (this.isAddress()) {
      if (this.subproof.length !== 0) return false;

      if (this.subindex !== 0) return false;

      if (this.proof.length > FAUCET_DEPTH) return false;

      if (this.index >= FAUCET_LEAVES) return false;

      return true;
    }

    if (this.subproof.length > AIRDROP_SUBDEPTH) return false;

    if (this.subindex >= AIRDROP_SUBLEAVES) return false;

    if (this.proof.length > AIRDROP_DEPTH) return false;

    if (this.index >= AIRDROP_LEAVES) return false;

    if (this.getSize() > MAX_PROOF_SIZE) return false;
  ...
  verify(expect) {
    ...
    if (!this.verifyMerkle(expect)) return false;
  ...
  fromJSON(json) {
    ...
    for (const hash of json.proof) this.proof.push(base16.decode(hash, 32));
    ...
    if (json.key) this.key = AirdropKey.fromJSON(json.key).encode();
