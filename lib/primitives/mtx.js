...
class MTX extends TX {
  ...
  addOutput(addr, value) {
    ...
    if (value != null) output = Output.fromScript(addr, value);
    else output = Output.fromOptions(addr);
  ...
  subtractFee(fee) {
    ...
    for (const output of this.outputs) {
      if (output.covenant.type !== types.NONE) continue;

      // Ignore nulldatas and
      // other OP_RETURN scripts.
      if (output.address.isUnspendable()) continue;
  ...
  sortMembers() {
    ...
    for (; i < this.inputs.length; i++) inputs.push(this.inputs[i]);
  ...
  setLocktime(locktime, seconds) {
    assert(
      Number.isSafeInteger(locktime) && locktime >= 0,
    ...
    if (seconds) {
      ...
      assert(
        (locktime & consensus.LOCKTIME_MASK) === locktime,
...
class CoinSelector {
  ...
  isSpendable(coin) {
    ...
    if (coin.covenant.isNonspendable()) return false;
    ...
    if (coin.coinbase) {
      ...
      if (this.height + 1 < coin.height + this.coinbaseMaturity) return false;
