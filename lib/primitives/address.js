...
class Address extends bio.Struct {
  ...
  getSigops(witness) {
    if (this.version === 0) {
      if (this.hash.length === 20) return 1;
  ...
  equals(addr) {
    ...
    return this.version === addr.version && this.hash.equals(addr.hash);
  }
  ...
  isValid() {
    ...
    if (this.hash.length < 2 || this.hash.length > 40) return false;
  ...
  format() {
    return (
      "<Address:" + ` version=${this.version}` + ` str=${this.toString()}` + ">"
    );
  }
  ...
  static getHash(data, network) {
    ...
    if (data instanceof Address) return data.hash;
