...
class KeyRing extends bio.Struct {
  ...
  fromOptions(options) {
    ...
    if (script) return this.fromScript(key, script);
  ...
  fromPublic(key) {
    ...
    assert(
      secp256k1.publicKeyVerify(key) && key.length === 33,
  ...
  fromKey(key) {
    ...
    if (key.length === 32) return this.fromPrivate(key, true);
  ...
  getScriptHash() {
    ...
    if (!this._scriptHash) this._scriptHash = this.script.sha3();
  ...
  getHash() {
    if (this.script) return this.getScriptHash();
  ...
  getAddress() {
    if (this.script) return this.getScriptAddress();
  ...
  getRedeem(hash) {
    if (this.script) {
      if (hash.equals(this.getScriptHash())) return this.script;
    }
  ...
  write(bw) {
    ...
    if (this.script) bw.writeVarBytes(this.script.encode());
  ...
  read(br) {
    ...
    if (script.length > 0) this.script = Script.decode(script);
