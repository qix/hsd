...
class ContractState {
  ...
  removeMap(map, nameHash, hash) {
    ...
    if (set.size === 0) map.delete(nameHash);
  ...
  track(tx, view) {
    ...
    if (view.names.size === 0) return this;
    ...
    for (const [nameHash, ns] of view.names) {
      ...
      if (this.names.has(nameHash)) continue;
  ...
  untrack(tx) {
    ...
    for (const nameHash of names) this.dereference(nameHash);
  ...
  merge(view) {
    for (const [nameHash, ns] of view.names) {
      if (!this.refs.has(nameHash)) continue;
  ...
  toSet(map, nameHash, items) {
    ...
    if (!hashes) return this;

    for (const hash of hashes) items.add(hash);
