...
class Mempool extends EventEmitter {
  ...
  async _addBlock(block, txs, view) {
    ...
    for (let i = 1; i < cb.inputs.length; i++) {
      ...
      if (!covenant.isClaim()) {
        ...
        if (entry) this.untrackAirdrop(entry);
      ...
      if (entry) this.untrackClaim(entry);
    }
    ...
    if (block.height + 1 === this.network.goosigStop) {
      for (const [hash, entry] of this.airdrops.entries()) {
        ...
        if (key.isGoo()) this.untrackAirdrop(entry);
      }
    }
  ...
  async _handleReorg() {
    ...
    for (const entry of this.claims.values()) {
      ...
      if (
        this.chain.tip.time < entry.inception ||
  ...
  limitSize(added) {
    ...
    while (queue.size() > 0) {
      ...
      this.logger.debug("Removing package %x from mempool (low fee).", hash);
    ...
    for (const entry of this.claims.values()) pq.insert(entry);
    ...
    for (const entry of this.airdrops.values()) pqa.insert(entry);
  ...
  getCoinsByAddress(addrs) {
    ...
    for (const addr of addrs) {
      ...
      for (const coin of coins) out.push(coin);
    }
  ...
  has(hash) {
    ...
    if (this.hasClaim(hash)) return true;

    if (this.hasAirdrop(hash)) return true;
  ...
  async addClaim(claim, id) {
    if (this.chain.height + 1 < this.network.txStart) {
      throw new VerifyError(claim, "invalid", "no-tx-allowed-yet", 0);
    }
  ...
  async insertClaim(claim, id) {
    ...
    if (this.exists(hash)) {
      throw new VerifyError(claim, "alreadyknown", "txn-already-in-mempool", 0);
    }
    ...
    if (!name) throw new VerifyError(claim, "invalid", "bad-claim-proof", 100);
    ...
    if (!data) throw new VerifyError(claim, "invalid", "bad-claim-data", 100);
    ...
    if (!(await this.verifyCovenants(tx, view, height, hardened))) {
      throw new VerifyError(tx, "invalid", "invalid-covenant", 0);
    }
    ...
    if (this.limitSize(hash)) {
      throw new VerifyError(claim, "insufficientfee", "mempool full", 0);
    }
  ...
  dropClaims() {
    ...
    for (const entry of this.claims.values()) entries.push(entry);

    for (const entry of entries) this.untrackClaim(entry);
  }
  ...
  dropAirdrops() {
    ...
    for (const entry of this.airdrops.values()) entries.push(entry);

    for (const entry of entries) this.untrackAirdrop(entry);
  }
  ...
  async addAirdrop(proof, id) {
    if (this.chain.height + 1 < this.network.txStart) {
      throw new VerifyError(proof, "invalid", "no-tx-allowed-yet", 0);
    }
  ...
  async insertAirdrop(proof, id) {
    ...
    // We can maybe ignore this.
    if (this.exists(hash)) {
      throw new VerifyError(proof, "alreadyknown", "txn-already-in-mempool", 0);
    }
    ...
    if (this.chain.height + 1 >= this.network.goosigStop) {
      ...
      if (!key) throw new VerifyError(proof, "invalid", "bad-airdrop-proof", 0);
    ...
    // Trim size if we're too big.
    if (this.limitSize(hash)) {
      throw new VerifyError(proof, "insufficientfee", "mempool full", 0);
    }
  ...
  async addTX(tx, id) {
    if (this.chain.height + 1 < this.network.txStart) {
      throw new VerifyError(tx, "invalid", "no-tx-allowed-yet", 0);
    }
  ...
  async insertTX(tx, id) {
    ...
    if (this.contracts.hasNames(tx)) {
      throw new VerifyError(tx, "alreadyknown", "name-already-in-mempool", 0);
    }
  ...
  trackEntry(entry, view) {
    ...
    if (view) this.contracts.track(entry.tx, view);
  }
  ...
  indexEntry(entry, view) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) this.coinIndex.insert(tx, i);
  }
  ...
  unindexEntry(entry) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) this.coinIndex.remove(hash, i);
  }
  ...
  removeDoubleOpens(tx) {
    ...
    for (const entry of entries) this.evictEntry(entry);
  }
...
class TXIndex {
  ...
  get(addr) {
    ...
    for (const entry of items.values()) out.push(entry.tx);
...
class CoinIndex {
  ...
  get(addr) {
    ...
    for (const coin of items.values()) out.push(coin.toCoin());
...
class MempoolCache {
  ...
  async getTip() {
    ...
    if (!tip) return consensus.ZERO_HASH;
  ...
  async verify() {
    ...
    if (version === -1) {
      ...
      this.logger.info("Mempool cache is empty. Writing tip %x.", tip);
