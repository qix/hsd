...
class NameState extends bio.Struct {
  ...
  get delta() {
    if (!this._delta) this._delta = new NameDelta();
    return this._delta;
  }
  ...
  state(height, network) {
    ...
    if (this.revoked !== 0) return states.REVOKED;

    if (this.claimed !== 0) {
      if (height < this.height + lockupPeriod) return states.LOCKED;
      return states.CLOSED;
    }

    if (height < this.height + openPeriod) return states.OPENING;
  ...
  isExpired(height, network) {
    ...
    if (this.revoked !== 0) {
      if (height < this.revoked + network.names.auctionMaturity) return false;
      return true;
    }

    // Can only renew once we reach the closed state.
    if (!this.isClosed(height, network)) return false;

    // Claimed names can only expire once the claim period is over.
    if (this.isClaimable(height, network)) return false;

    // If we haven't been renewed in a year, start over.
    if (height >= this.renewal + network.names.renewalWindow) return true;

    // If nobody revealed their bids, start over.
    if (this.owner.isNull()) return true;
  ...
  setHeight(height) {
    ...
    if (this.delta.height === null) this.delta.height = this.height;
  ...
  setRenewal(renewal) {
    ...
    if (renewal === this.renewal) return this;

    if (this.delta.renewal === null) this.delta.renewal = this.renewal;
  ...
  setOwner(owner) {
    ...
    if (owner.equals(this.owner)) return this;

    if (this.delta.owner === null) this.delta.owner = this.owner;
  ...
  setValue(value) {
    ...
    if (this.delta.value === null) this.delta.value = this.value;
  ...
  setHighest(highest) {
    ...
    if (highest === this.highest) return this;

    if (this.delta.highest === null) this.delta.highest = this.highest;
  ...
  setData(data) {
    ...
    if (this.delta.data === null) this.delta.data = this.data;
  ...
  setTransfer(transfer) {
    ...
    if (transfer === this.transfer) return this;

    if (this.delta.transfer === null) this.delta.transfer = this.transfer;
  ...
  setRevoked(revoked) {
    ...
    if (revoked === this.revoked) return this;

    if (this.delta.revoked === null) this.delta.revoked = this.revoked;
  ...
  setClaimed(claimed) {
    ...
    if (claimed === this.claimed) return this;

    if (this.delta.claimed === null) this.delta.claimed = this.claimed;
  ...
  setRenewals(renewals) {
    ...
    if (renewals === this.renewals) return this;

    if (this.delta.renewals === null) this.delta.renewals = this.renewals;
  ...
  setRegistered(registered) {
    ...
    if (registered === this.registered) return this;

    if (this.delta.registered === null) this.delta.registered = this.registered;
  ...
  setExpired(expired) {
    ...
    if (expired === this.expired) return this;

    if (this.delta.expired === null) this.delta.expired = this.expired;
  ...
  setWeak(weak) {
    ...
    if (weak === this.weak) return this;

    if (this.delta.weak === null) this.delta.weak = this.weak;
  ...
  applyState(delta) {
    ...
    if (delta.height !== null) this.height = delta.height;

    if (delta.renewal !== null) this.renewal = delta.renewal;

    if (delta.owner !== null) this.owner = delta.owner;

    if (delta.value !== null) this.value = delta.value;

    if (delta.highest !== null) this.highest = delta.highest;

    if (delta.data !== null) this.data = delta.data;

    if (delta.transfer !== null) this.transfer = delta.transfer;

    if (delta.revoked !== null) this.revoked = delta.revoked;

    if (delta.claimed !== null) this.claimed = delta.claimed;

    if (delta.renewals !== null) this.renewals = delta.renewals;

    if (delta.registered !== null) this.registered = delta.registered;

    if (delta.expired !== null) this.expired = delta.expired;

    if (delta.weak !== null) this.weak = delta.weak;
  ...
  getSize() {
    ...
    if (this.value !== 0) size += encoding.sizeVarint(this.value);

    if (this.highest !== 0) size += encoding.sizeVarint(this.highest);

    if (this.transfer !== 0) size += 4;

    if (this.revoked !== 0) size += 4;

    if (this.claimed !== 0) size += 4;

    if (this.renewals !== 0) size += encoding.sizeVarint(this.renewals);
  ...
  getField() {
    ...
    if (!this.owner.isNull()) field |= 1 << 0;

    if (this.value !== 0) field |= 1 << 1;

    if (this.highest !== 0) field |= 1 << 2;

    if (this.transfer !== 0) field |= 1 << 3;

    if (this.revoked !== 0) field |= 1 << 4;

    if (this.claimed !== 0) field |= 1 << 5;

    if (this.renewals !== 0) field |= 1 << 6;

    if (this.registered) field |= 1 << 7;

    if (this.expired) field |= 1 << 8;

    if (this.weak) field |= 1 << 9;
  ...
  write(bw) {
    ...
    if (this.value !== 0) bw.writeVarint(this.value);

    if (this.highest !== 0) bw.writeVarint(this.highest);

    if (this.transfer !== 0) bw.writeU32(this.transfer);

    if (this.revoked !== 0) bw.writeU32(this.revoked);

    if (this.claimed !== 0) bw.writeU32(this.claimed);

    if (this.renewals !== 0) bw.writeVarint(this.renewals);
  ...
  read(br) {
    ...
    if (field & (1 << 1)) this.value = br.readVarint();

    if (field & (1 << 2)) this.highest = br.readVarint();

    if (field & (1 << 3)) this.transfer = br.readU32();

    if (field & (1 << 4)) this.revoked = br.readU32();

    if (field & (1 << 5)) this.claimed = br.readU32();

    if (field & (1 << 6)) this.renewals = br.readVarint();

    if (field & (1 << 7)) this.registered = true;

    if (field & (1 << 8)) this.expired = true;

    if (field & (1 << 9)) this.weak = true;
...
// Max size: 668
NameState.MAX_SIZE =
  0 + 1 + 63 + 2 + 512 + 4 + 4 + 2 + 32 + 9 + 9 + 9 + 4 + 4 + 4 + 9;
