...
class Field {
  ...
  set(i, val) {
    ...
    if (val) this.field[i >>> 3] |= 1 << (7 - (i & 7));
    else this.field[i >>> 3] &= ~(1 << (7 - (i & 7)));
...
class BitView {
  ...
  spend(field, tx) {
    ...
    for (let i = 1; i < tx.inputs.length; i++) {
      ...
      if (!covenant.isNone()) continue;

      const proof = AirdropProof.decode(witness.items[0]);
      const index = proof.position();

      if (!this.bits.has(index)) this.bits.set(index, field.get(index));
  ...
  commit(field) {
    ...
    for (const [bit, spent] of this.bits) field.set(bit, spent);
