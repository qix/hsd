...
class Script extends bio.Struct {
  ...
  fromOptions(options) {
    ...
    if (Buffer.isBuffer(options)) return this.decode(options);

    if (Array.isArray(options)) return this.fromArray(options);

    if (options.raw) {
      if (!options.code) return this.decode(options.raw);
  ...
  compile() {
    ...
    for (const op of this.code) op.write(bw);
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKSIGVERIFY: {
          ...
          if (sig.length > 0) {
            ...
            const hash = tx.signatureHash(index, subscript, value, type);
  ...
  isPubkey(minimal) {
    ...
    return this.getLength(0) === 33 && this.getOp(1) === opcodes.OP_CHECKSIG;
  }
  ...
  isMultisig(minimal) {
    ...
    for (let i = 1; i < n + 1; i++) {
      ...
      if (op.toLength() !== 33) return false;
  ...
  static verify(witness, addr, tx, index, value, flags) {
    ...
    if (addr.version === 31) throw new ScriptError("OP_RETURN");
  ...
  decode(data) {
    ...
    while (br.left()) this.code.push(Opcode.read(br));
...
function validateKey(key, flags) {
  ...
  if (!common.isKeyEncoding(key)) throw new ScriptError("PUBKEY_ENCODING");
...
function validateSignature(sig, flags) {
  ...
  if (!common.isSignatureEncoding(sig)) throw new ScriptError("SIG_ENCODING");
