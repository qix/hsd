...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get("/mempool", async (req, res) => {
      ...
      for (const hash of hashes) result.push(hash.toString("hex"));
  ...
  handleAuth(socket) {
    ...
    socket.hook("get name", async (...args) => {
      ...
      if (!nameHash) throw new Error("Invalid parameter.");
  ...
  filterBlock(socket, block) {
    ...
    for (const tx of block.txs) {
      if (this.filterTX(socket, tx)) txs.push(tx.encode());
    }
  ...
  async scan(socket, start) {
    await this.node.scan(start, socket.filter, (entry, txs) => {
      ...
      for (const tx of txs) raw.push(tx.encode());
