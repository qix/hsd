...
class RPC extends RPCBase {
  ...
  async getPeerInfo(args, help) {
    ...
    for (let peer = this.pool.peers.head(); peer; peer = peer.next) {
      ...
      if (type && peer.outbound !== (type === "outbound")) continue;
  ...
  async verifyTXOutProof(args, help) {
    ...
    for (const hash of tree.matches) out.push(hash.toString("hex"));
  ...
  async _handleWork(data) {
    ...
    if (!attempt) return [false, "no-mining-job"];

    if (data.length !== 256) return [false, "invalid-data-length"];

    const hdr = Headers.fromMiner(data);

    if (!hdr.prevBlock.equals(attempt.prevBlock) || hdr.bits !== attempt.bits) {
      return [false, "stale"];
    }

    const mask = this.maskMap.get(hdr.maskHash());

    if (!mask) return [false, "stale"];
    ...
    if (!entry) {
      this.logger.warning(
        "RPC block rejected: %x (bad-prevblk).",
        block.hash()
      );
  ...
  async getWork(args, help) {
    ...
    if (args.length === 1) {
      ...
      if (!maskHash) throw new RPCError(errs.TYPE_ERROR, "Invalid mask hash.");

      if (this.maskMap.has(maskHash)) return null;
    }
  ...
  async getBlockTemplate(args, help) {
    ...
    if (!this.network.selfConnect) {
      if (this.pool.peers.size() === 0) {
        throw new RPCError(errs.CLIENT_NOT_CONNECTED, "Node is not connected!");
      }
  ...
  async signRawTransaction(args, help) {
    ...
    if (sighash) {
      ...
      if (parts.length === 2) {
        ...
        if (parts[1] === "NOINPUT") type |= Script.hashType.NOINPUT;

        if (parts[1] === "ANYONECANPAY") type |= Script.hashType.ANYONECANPAY;
      }
    }
  ...
  async verifyMessage(args, help) {
    ...
    if (addr.version !== 0 || addr.hash.length !== 20) return false;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (safeEqual(blake2b.digest(key, 20), addr.hash)) return true;
    }
  ...
  async getNameInfo(args, help) {
    ...
    if (ns) {
      if (!ns.isExpired(height, network)) info = ns.getJSON(height, network);
    }
  ...
  async sendRawClaim(args, help) {
    ...
    if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid base64 string.");
  ...
  async validateResource(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(
        errs.MISC_ERROR,
        "validateresource" + " '{\"records\": [{...}]}'"
      );

    const valid = new Validator(args);
    const data = valid.obj(0);

    if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid resource object.");
  ...
  async getSoftforks() {
    ...
    for (const deployment of this.network.deploys) {
      ...
      if (status === "started") {
        forks[deployment.name].statistics = await this.chain.getBIP9Stats(
          tip,
          deployment
        );
      }
  ...
  async getHashRate(lookup, height) {
    ...
    if (lookup === 0) lookup = (tip.height % this.network.pow.targetWindow) + 1;
...
function parseIP(addr, network) {
  ...
  if (ip.port === 0) ip.port = ip.key ? network.brontidePort : network.port;
