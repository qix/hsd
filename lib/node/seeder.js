...
class Seeder extends DNSServer {
  ...
  initOptions(options) {
    ...
    if (options.zone != null) this.zone = util.fqdn(options.zone);

    if (options.ns != null) this.ns = util.fqdn(options.ns);

    if (options.ip != null) this.ip = IP.normalize(options.ip);
  ...
  async resolve(req, rinfo) {
    ...
    if (!util.isSubdomain(this.zone, name)) throw createError(codes.NOTZONE);

    if (!util.equal(name, this.zone)) return this.createEmpty(codes.NXDOMAIN);
  ...
  createA(type) {
    ...
    for (const entry of this.hosts.map.values()) {
      ...
      if (this.hosts.isStale(entry)) continue;

      if (!entry.lastSuccess) continue;

      if (addr.port !== this.hosts.network.port) continue;
    ...
    for (const entry of items) {
      ...
      if (res.answer.length === 50) break;
    }

    if (res.answer.length === 0) return this.createEmpty(codes.SUCCESS);
