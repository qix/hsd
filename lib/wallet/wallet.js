...
class Wallet extends EventEmitter {
  ...
  async makeBidMany(names, value, lockup, acct) {
    names.forEach((name) => {
      assert(typeof name === "string");
    });
  ...
  async _createBidMany(names, value, lockup, options) {
    const acct = options ? options.account || 0 : 0;
    const mtx = await this.makeBidMany(names, value, lockup, acct);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }

  async _sendBidMany(names, value, lockup, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createBidMany(names, value, lockup, options);
    return this.sendMTX(mtx, passphrase);
  }
  async sendBidMany(names, value, lockup, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendBidMany(names, value, lockup, options);
    } finally {
      unlock();
    }
  }
  ...
  async _addRegisterMtx(mtx, name, resource, acct) {
  ...
  async _makeRegister(name, resource) {
    ...
    const mtx = new MTX();
    await this._addRegisterMtx(mtx, name, resource);
   ...
   */
   ...
   async _addUpdateMtx(mtx, name, resource, acct) {
    const rawName = Buffer.from(name, "ascii");
    const nameHash = rules.hashName(rawName);
    const ns = await this.getNameState(nameHash);
    const height = this.wdb.height + 1;
    ...
    if (coin.covenant.isReveal() || coin.covenant.isClaim()) {
      await this._addRegisterMtx(mtx, name, resource, acct);
      return;
    }
  ...
  async makeUpdateMany(names, resource, acct) {
    assert(resource instanceof Resource);
    names.forEach(name => {
      assert(typeof name === "string");
      if (!rules.verifyName(name)) throw new Error("Invalid name.");
    ...
    const mtx = new MTX();
    for (const name of names) {
      await this._addUpdateMtx(mtx, name, resource, acct);
    }
  ...
  async _createUpdateMany(names, resource, options) {
    const acct = options ? options.account : null;
    const mtx = await this.makeUpdateMany(names, resource, acct);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendUpdateMany(names, resource, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createUpdateMany(names, resource, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendUpdateMany(names, resource, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendUpdateMany(names, resource, options);
    } finally {
      unlock();
    }
  }
  ...
  async makeTransferMany(names, address, acct) {
    names.forEach(name => {
      assert(typeof name === "string");
    });
    assert(address instanceof Address);
  ...
  async _createTransferMany(names, address, options) {
    const acct = options ? options.account : null;
    const mtx = await this.makeTransferMany(names, address, acct);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }

  /**
   * Create and finalize a transferMany
   * MTX with a lock.
   * @param {String} name
   * @param {Address} address
   * @param {Object} options
  ...
  async createTransferMany(names, address, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._createTransferMany(names, address, options);
    } finally {
      unlock();
    }
  }
  ...
  /**
   * Create and send a transferMany
   * MTX without a lock.
   * @param {String} name
   * @param {Address} address
   * @param {Object} options
  ...
  async _sendTransferMany(names, address, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createTransferMany(names, address, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendTransferMany(names, address, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendTransferMany(names, address, options);
    } finally {
      unlock();
    }
  }
  ...
  async makeFinalizeMany(names, acct) {
    for (const name of names) {
    assert(typeof name === "string");
  ...
  async _createFinalizeMany(names, options) {
    const acct = options ? options.account : null;
    const mtx = await this.makeFinalizeMany(names, acct);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }

  /**
   * Create and finalizeMany a finalizeMany
   * MTX with a lock.
   * @param {String} names
   * @param {Object} options
   * @returns {MTX}
  ...
  async createFinalizeMany(names, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._createFinalizeMany(names, options);
    } finally {
      unlock();
    }
  }
  ...
  /**
   * Create and send a finalizeMany
   * MTX without a lock.
   * @param {String} names
   * @param {Object} options
   */

  async _sendFinalizeMany(names, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createFinalizeMany(names, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendFinalizeMany(names, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendFinalizeMany(names, options);
    } finally {
      unlock();
    }
  }
