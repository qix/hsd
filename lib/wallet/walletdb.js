...
class WalletDB extends EventEmitter {
  ...
  async estimateFee(blocks) {
    ...
    if (rate < this.network.feeRate) return this.network.feeRate;

    if (rate > this.network.maxFeeRate) return this.network.maxFeeRate;
  ...
  async getWalletsByTX(tx) {
    ...
    if (!tx.isCoinbase()) {
      for (const { prevout } of tx.inputs) {
        ...
        if (!this.testFilter(prevout.encode())) continue;
    ...
    for (const { covenant } of tx.outputs) {
      ...
      if (!this.testFilter(nameHash)) continue;
  ...
  async _addBlock(entry, txs) {
    ...
    if (total > 0) {
      this.logger.info("Connected WalletDB block %x (tx=%d).", tip.hash, total);
    }
  ...
  async _removeBlock(entry) {
    ...
    this.logger.warning(
      "Disconnected wallet block %x (tx=%d).",
      tip.hash,
      total
    );
