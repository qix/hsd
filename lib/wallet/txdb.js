...
class TXDB {
  ...
  async add(tx, block) {
    ...
    if (!block) {
      ...
      if (await this.isDoubleOpen(tx)) return null;
    } else {
      // Potentially remove double-spenders.
  ...
  async insert(wtx, block) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (height !== -1) await this.lockBalances(b, state, tx, i, path, -1);
    ...
    if (block) {
      ...
      if (updated && !state.updated()) await b.write();
    }
  ...
  async erase(wtx, block) {
    ...
    if (!block) this.unindexOpens(b, tx);
  ...
  async lockBalances(b, state, tx, i, path, height) {
    ...
    switch (covenant.type) {
      case types.CLAIM:
      case types.BID: {
        if (height === -1) state.ulocked(path, output.value);
        else state.clocked(path, output.value);
        break;
      }
  ...
  async connectNames(b, tx, view, height) {
      ...
      switch (covenant.type) {
        ...
        case types.OPEN: {
          if (!path) {
            ...
            if (!map || !map.wids.has(this.wid)) break;
        ...
        case types.BID: {
          ...
          if (ns.isNull()) await this.addNameMap(b, nameHash);
    ...
    if (updated) {
      ...
      if (undo.names.length > 0) b.put(layout.U.encode(hash), undo.encode());
    }
  ...
  async zap(acct, age) {
    ...
    for (const wtx of txs) {
      ...
      this.logger.debug("Zapping TX: %x (%d)", wtx.tx.hash(), this.wid);
...
class DetailsMember {
  ...
  getJSON(network) {
    return {
      ...
      covenant: this.covenant ? this.covenant.toJSON() : undefined,
