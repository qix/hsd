...
class RPC extends RPCBase {
  ...
  async getReceivedByAddress(args, help) {
    ...
    for (const wtx of txs) {
      ...
      for (const output of wtx.tx.outputs) {
        if (output.getHash().equals(hash)) total += output.value;
      }
    }
  ...
  async importName(args, help) {
    if (help || args.length < 1 || args.length > 2) {
      throw new RPCError(errs.MISC_ERROR, 'importname "name" ( height )');
    }
    ...
    if (height != null) await this.wdb.rescan(height);
  ...
  async listAddressGroupings(args, help) {
    if (help) throw new RPCError(errs.MISC_ERROR, "listaddressgroupings");
  ...
  async sendMany(args, help) {
    ...
    if (!sendTo) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter.");
  ...
  async setAccount(args, help) {
    if (help || args.length < 1 || args.length > 2) {
      throw new RPCError(errs.MISC_ERROR, 'setaccount "address" "account"');
    }
  ...
  async signMessage(args, help) {
    if (help || args.length !== 2) {
      throw new RPCError(errs.MISC_ERROR, 'signmessage "address" "message"');
    }
  ...
  async importPrunedFunds(args, help) {
    ...
    if (height === -1) throw new RPCError(errs.VERIFY_ERROR, "Invalid proof.");
  ...
  async getBids(args, help) {
    ...
    for (const bid of bids) {
      if (!own || bid.own) items.push(bid.toJSON());
    }
  ...
  async getReveals(args, help) {
    ...
    for (const brv of reveals) {
      if (!own || brv.own) items.push(brv.toJSON());
    }
  ...
  async getNames(args, help) {
    ...
    for (const ns of names) items.push(ns.getJSON(height, network));
  ...
  async getAuctionInfo(args, help) {
    ...
    if (!ns) throw new RPCError(errs.MISC_ERROR, "Auction not found.");
    ...
    for (const bid of bids) info.bids.push(bid.toJSON());

    for (const reveal of reveals) info.reveals.push(reveal.toJSON());
  ...
  async getNameResource(args, help) {
    ...
    if (!ns || ns.data.length === 0) return null;
  ...
  async getNameByHash(args, help) {
    ...
    if (!hash) throw new RPCError(errs.TYPE_ERROR, "Invalid name hash.");
  ...
  async sendBidMany(args, help) {
    const account = null;
    const bid = 1550097;
    const value = 1550097;
    const wallet = this.wallet;
    const tx = await wallet.sendBidMany(args, bid, value, {
      account: account,
    });
  ...
  async sendUpdateMany(args, help) {
    const wallet = this.wallet;
    const resource = Resource.fromJSON({"records":[]});
    const tx = await wallet.sendUpdateMany(args, resource, {
      account: null,
    });
  ...
  async sendTransferMany(args, help) {
    const address = parseAddress(args[0], this.network);
    const names = args.slice(1);
    const wallet = this.wallet;
    const tx = await wallet.sendTransferMany(names, address, {
      account: null,
    });
  ...
  _validateTransfer(args, help, method) {
    ...
    if (!addr) throw new RPCError(errs.TYPE_ERROR, "Invalid address.");
  ...
  async sendFinalizeMany(args, help) {
    const wallet = this.wallet;
    const tx = await wallet.sendFinalizeMany(args, {
      account: null,
    });
  ...
  async importNonce(args, help) {
    ...
    if (addr == null) throw new RPCError(errs.TYPE_ERROR, "Invalid value.");

    if (value == null) throw new RPCError(errs.TYPE_ERROR, "Invalid value.");
