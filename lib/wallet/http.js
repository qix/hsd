...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get("/wallet/:id/bid/:name", async (req, res) => {
      ...
      if (name) assert(rules.verifyName(name), "Must pass valid name.");
    ...
    this.get("/wallet/:id/resource/:name", async (req, res) => {
      ...
      if (!ns || ns.data.length === 0) return res.json(404);
    ...
    this.post("/wallet/:id/open", async (req, res) => {
      ...
      if (sign) await req.wallet.sign(mtx, passphrase);
  ...
  initSockets() {
    ...
    this.wdb.on("address", (wallet, receive) => {
      ...
      for (const addr of receive) json.push(addr.getJSON(this.network));
...
class TransactionOptions {
  ...
  constructor(valid) {
    if (valid) return this.fromValidator(valid);
  }
  ...
  fromValidator(valid) {
    ...
    if (valid.has("outputs")) {
      ...
      for (const output of outputs) {
        ...
        if (covenant) covenant = Covenant.fromJSON(covenant);
