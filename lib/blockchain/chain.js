...
class Chain extends AsyncEmitter {
  ...
  async open() {
    ...
    if (!this.options.spv) await this.syncTree();
  ...
  async verify(block, prev, flags) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (opens > consensus.MAX_BLOCK_OPENS) {
        throw new VerifyError(block, "invalid", "bad-blk-updates", 100);
      }
    ...
    for (let i = 1; i < cb.inputs.length; i++) {
      ...
      if (witness.items.length !== 1) {
        throw new VerifyError(block, "invalid", "bad-witness-size", 100);
      }

      if (i >= cb.outputs.length) {
        throw new VerifyError(block, "invalid", "bad-output", 100);
      }
      ...
      if (!output.covenant.isClaim()) {
        ...
        try {
          ...
          throw new VerifyError(block, "invalid", "bad-airdrop-format", 100);
        }

        if (!proof.isSane()) {
          throw new VerifyError(block, "invalid", "bad-airdrop-sanity", 100);
        }
        ...
        if (prev.height + 1 >= this.network.goosigStop) {
          ...
          if (!key) {
            throw new VerifyError(block, "invalid", "bad-airdrop-proof", 100);
          }

          if (key.isGoo()) {
            throw new VerifyError(block, "invalid", "bad-goosig-disabled", 100);
          }
        }
        ...
        if (state.hasHardening()) {
          if (proof.isWeak()) {
            throw new VerifyError(block, "invalid", "bad-airdrop-sanity", 10);
          }
        }
      ...
      try {
        ...
        throw new VerifyError(block, "invalid", "bad-dnssec-format", 100);
      }

      // Verify times.
      if (!proof.verifyTimes(prev.time)) {
        throw new VerifyError(block, "invalid", "bad-dnssec-times", 10);
      }
    }
  ...
  async verifyInputs(block, prev, state) {
    ...
    if (!block.treeRoot.equals(this.db.treeRoot())) {
      throw new VerifyError(block, "invalid", "bad-tree-root", 100);
    }
  ...
  async getMainHeight(hash) {
    ...
    if (!(await this.db.isMainChain(entry))) return -1;
  ...
  async verifyRenewal(hash, height) {
    ...
    // Cannot renew yet.
    if (height < this.network.names.renewalMaturity) return true;
    ...
    // Must be the current chain.
    if (!(await this.db.isMainChain(entry))) return false;
    ...
    if (entry.height < height - this.network.names.renewalPeriod) return false;
  ...
  async verifyCovenants(tx, view, height, hardened) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (covenant.isClaim()) {
        ...
        if (!valid) {
          throw new VerifyError(tx, "invalid", "bad-claim-state", 100);
        }
        ...
        if (ns.expired || !rules.isReserved(nameHash, height, network)) {
          throw new VerifyError(tx, "invalid", "bad-claim-notreserved", 100);
        }
        ...
        if (hardened && weak) {
          throw new VerifyError(tx, "invalid", "bad-claim-algorithm", 100);
        }
        ...
        if (claimed !== covenant.getU32(5)) {
          throw new VerifyError(tx, "invalid", "bad-claim-commit-height", 100);
        }

        // Implicitly disallows the genesis block.
        if (claimed <= ns.claimed) {
          throw new VerifyError(tx, "invalid", "bad-claim-commit-hash", 100);
        }
      ...
      if (covenant.isOpen()) {
        if (state !== states.OPENING) {
          throw new VerifyError(tx, "invalid", "bad-open-state", 100);
        }

        // Only one open transaction can ever exist.
        if (ns.height !== height) {
          throw new VerifyError(tx, "invalid", "bad-open-multiple", 100);
        }

        // Cannot bid on a reserved name.
        if (!ns.expired && rules.isReserved(nameHash, height, network)) {
          throw new VerifyError(tx, "invalid", "bad-open-reserved", 100);
        }

        // On mainnet, names are released on a
        // weekly basis for the first year.
        if (!rules.hasRollout(nameHash, height, network)) {
          throw new VerifyError(tx, "invalid", "bad-open-rollout", 100);
        }
      ...
      if (covenant.isBid()) {
        if (state !== states.BIDDING) {
          throw new VerifyError(tx, "invalid", "bad-bid-state", 100);
        }

        if (start !== ns.height) {
          throw new VerifyError(tx, "invalid", "bad-bid-height", 100);
        }
      ...
      switch (covenant.type) {
        ...
        case types.REVEAL: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-reveal-nonlocal", 100);
          }

          // Early reveals? No.
          if (state !== states.REVEAL) {
            throw new VerifyError(tx, "invalid", "bad-reveal-state", 100);
          }
        ...
        case types.REDEEM: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-redeem-nonlocal", 100);
          }

          // Allow participants to get their
          // money out, even in a revoked state.
          if (state < states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-redeem-state", 100);
          }

          // Must be the loser in order
          // to redeem the money now.
          if (prevout.equals(ns.owner)) {
            throw new VerifyError(tx, "invalid", "bad-redeem-owner", 100);
          }
        ...
        case types.REGISTER: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-register-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-register-state", 100);
          }
          ...
          if (!(await this.verifyRenewal(hash, height))) {
            throw new VerifyError(tx, "invalid", "bad-register-renewal", 100);
          }

          // Must be the winner in
          // order to redeem the name.
          if (!prevout.equals(ns.owner)) {
            throw new VerifyError(tx, "invalid", "bad-register-owner", 100);
          }

          // Must match the second highest bid.
          if (output.value !== ns.value) {
            throw new VerifyError(tx, "invalid", "bad-register-value", 100);
          }
          ...
          if (data.length > 0) ns.setData(data);
        ...
        case types.UPDATE: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-update-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-update-state", 100);
          }
        ...
        case types.RENEW: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-renewal-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-renewal-state", 100);
          }

          const hash = covenant.getHash(2);

          if (height < ns.renewal + network.names.treeInterval) {
            throw new VerifyError(tx, "invalid", "bad-renewal-premature", 100);
          }

          if (!(await this.verifyRenewal(hash, height))) {
            throw new VerifyError(tx, "invalid", "bad-renewal", 100);
          }
        ...
        case types.TRANSFER: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-transfer-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-transfer-state", 100);
          }
        ...
        case types.FINALIZE: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-finalize-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-finalize-state", 100);
          }
          ...
          if (height < ns.transfer + network.names.transferLockup) {
            throw new VerifyError(tx, "invalid", "bad-finalize-maturity", 100);
          }
          ...
          if (!(await this.verifyRenewal(hash, height))) {
            throw new VerifyError(tx, "invalid", "bad-finalize-renewal", 100);
          }
        ...
        case types.REVOKE: {
          if (start !== ns.height) {
            throw new VerifyError(tx, "invalid", "bad-revoke-nonlocal", 100);
          }

          if (state !== states.CLOSED) {
            throw new VerifyError(tx, "invalid", "bad-revoke-state", 100);
          }
  ...
  async connect(prev, block, flags) {
    ...
    if (prev.height + 1 < this.network.txStart) {
      ...
      if (block.txs.length > 1) invalid = true;

      if (!this.options.spv) {
        // No claims or airdrops allowed in coinbase yet.
        const cb = block.txs[0];
        if (cb.outputs.length > 1) invalid = true;

        // Sanity check
        if (!cb.outputs[0].covenant.isNone()) invalid = true;
      }
  ...
  storeOrphan(block, flags, id) {
    ...
    this.logger.debug("Storing orphan block: %x (%d).", block.hash(), height);
  ...
  async getSuitableBlock(prev) {
    ...
    if (x.time > z.time) [x, z] = [z, x];

    if (x.time > y.time) [x, y] = [y, x];

    if (y.time > z.time) [y, z] = [z, y];
  ...
  async getTarget(time, prev) {
    ...
    // Do not retarget
    if (pow.noRetargeting) return pow.bits;
  ...
  retarget(first, last) {
    ...
    if (work.isZero()) return pow.bits;
  ...
  async readDeploymentState(tip) {
    ...
    if (this.options.spv) return new DeploymentState(this.network.genesis.hash);
  ...
  async verifyFinal(prev, tx, flags) {
    ...
    if (!(tx.locktime & consensus.LOCKTIME_FLAG)) return tx.isFinal(height, -1);
...
class ChainOptions {
  ...
  fromOptions(options) {
    ...
    if (this.spv || this.memory) this.treePrefix = null;
