...
class ChainDB {
  ...
  async migrateChainState() {
    if (await this.db.has(layout.M.encode(0))) return;
      ...
      for (let i = 0; i < block.txs.length; i++) {
        ...
        if (i > 0) {
          for (const { prevout } of tx.inputs) {
            ...
            if (view.get(hash).outputs.size === 0) assert(view.remove(hash));
        ...
        for (let i = 0; i < tx.outputs.length; i++) {
          ...
          if (output.covenant.isRegister()) pending.burn(output);
  ...
  async lookup(root, key) {
    if (this.options.spv) throw new Error("Cannot lookup in SPV mode.");
  ...
  async prove(root, key) {
    if (this.options.spv) throw new Error("Cannot prove in SPV mode.");
  ...
  async saveGenesis() {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (i > 0) assert(await view.spendInputs(this, tx));
  ...
  async getNameStatus(nameHash, height, hardened) {
    if (hardened == null) hardened = false;
  ...
  async hasTX(hash) {
    if (!this.options.indexTX) return false;
  ...
  async getHashesByAddress(addrs) {
    if (!this.options.indexTX || !this.options.indexAddress) return [];
  ...
  async scan(start, filter, iter) {
    ...
    else this.logger.info("Scanning from block %x.", start);
    ...
    while (entry) {
      ...
      this.logger.info("Scanning block %x (%d).", entry.hash, entry.height);

      for (const tx of block.txs) {
        if (tx.test(filter)) txs.push(tx);
      }
  ...
  async saveNames(view, entry, revert) {
    ...
    if (entry.height % this.network.names.treeInterval === 0) {
      ...
      if (revert) await this.tree.inject(entry.treeRoot);
      else await this.txn.commit();
  ...
  async connectNames(view, entry) {
    ...
    if (undo.names.length === 0) this.del(layout.w.encode(entry.height));
    else this.put(layout.w.encode(entry.height), undo.encode());
  ...
  async connectBlock(entry, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      for (let i = 0; i < tx.outputs.length; i++) {
        ...
        if (output.covenant.isRegister()) this.pending.burn(output);
  ...
  async disconnectBlock(entry, block) {
    ...
    for (let i = block.txs.length - 1; i >= 0; i--) {
      ...
      for (let j = tx.outputs.length - 1; j >= 0; j--) {
        ...
        // Registers are burned.
        if (output.covenant.isRegister()) this.pending.unburn(output);
...
class ChainFlags extends bio.Struct {
  ...
  write(bw) {
    ...
    if (this.prune) flags |= 1 << 1;

    if (this.indexTX) flags |= 1 << 2;

    if (this.indexAddress) flags |= 1 << 3;
