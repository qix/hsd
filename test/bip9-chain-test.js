...
async function addBIP9Blocks(number, setHardeningBit) {
  for (let i = 0; i < number; i++) {
    ...
    if (setHardeningBit) job.attempt.version |= 1 << deployments.hardening.bit;
    else job.attempt.version = 0;
    job.refresh();
...
describe("BIP9 activation", function () {
  ...
  it("should throw error if not STARTED", async () => {
    await assert.rejects(chain.getBIP9Stats(chain.tip, deployments.hardening), {
